cd ../java/src
mkdir -p /data/sidana/purch/fm/len1/rv
javac preProcess/ConvertIntoRelVecGeneralized.java
	...
	filename3="/data/sidana/purch/fm/len1/rv/relevanceVector_$countryCode"
	java -cp . preProcess.ConvertIntoRelVecGeneralized "$filename1" "$filename2" "$filename3" 5
    dur=$(echo "$(date +%s.%N) - $start" | bc)
    echo "$countryCode:$dur">>/data/sidana/purch/fm/len1/rv/convertintorelvecgeneralized.txt
cd -
