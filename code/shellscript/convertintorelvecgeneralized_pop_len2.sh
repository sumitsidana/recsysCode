cd ../java
mkdir -p /data/sidana/recsysBaselines/experiments/popcountryfiles/output/len2/rv/
...
for countryCode in "at" "be" "br" "ch" "cz" "de" "dk" "es" "fi" "ie" "nb" "nl" "no" "pl" "pt" "ru" "se" "uk" "it" "fr"; do
	...
	filename3="/data/sidana/recsysBaselines/experiments/popcountryfiles/output/len2/rv/relevanceVector_$countryCode$a"
	java -cp . preProcess.ConvertIntoRelVecGeneralized "$filename1" "$filename2" "$filename3" 15
    dur=$(echo "$(date +%s.%N) - $start" | bc)
    echo "$countryCode:$dur">>/data/sidana/recsysBaselines/experiments/popcountryfiles/output/len2/convertintorelvecgeneralized.txt
done
cd -
